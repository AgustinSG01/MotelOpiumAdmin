...
function EditSuit({ handleClose, open, suit, refresh }: NewEmployeeProps): React.JSX.Element {
  ...
  async function handleSubmit(): Promise<void> {
    if (validate()) {
      ...
      try {
        const newSuit = await instance.put(`/suit/${suit?.id}`, data);
        if (newSuit) {
          handleClose();
          setAlert({
            show: true,
            message: 'Suit atualizado',
            type: 'success',
          });
          refresh();
        }
